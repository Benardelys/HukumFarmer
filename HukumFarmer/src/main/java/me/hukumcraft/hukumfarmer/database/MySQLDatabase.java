...
/**
 * MySQL database backend using HikariCP for high-player-count servers.
 */
public class MySQLDatabase implements DatabaseManager {
    ...
    public MySQLDatabase(HukumFarmer plugin) {
        this.plugin = plugin;
    }
    ...
    public void init() throws SQLException {
        ...
        String database = config.getString("mysql.database", "hukumfarmer");
        String username = config.getString("mysql.username", "root");
        String password = config.getString("mysql.password", "password");

        HikariConfig hikariConfig = new HikariConfig();
        hikariConfig.setPoolName("HukumFarmer-MySQL-Pool");
        hikariConfig.setJdbcUrl("jdbc:mysql://" + host + ":" + port + "/" + database + "?useSSL=false&allowPublicKeyRetrieval=true&useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC");
        hikariConfig.setUsername(username);
        hikariConfig.setPassword(password);

        hikariConfig.setMaximumPoolSize(config.getInt("mysql.pool.maximum-pool-size", 10));
        hikariConfig.setMinimumIdle(config.getInt("mysql.pool.minimum-idle", 5));
        hikariConfig.setMaxLifetime(config.getLong("mysql.pool.max-lifetime", 1800000));
        hikariConfig.setConnectionTimeout(config.getLong("mysql.pool.connection-timeout", 10000));
        hikariConfig.setLeakDetectionThreshold(config.getLong("mysql.pool.leak-detection-threshold", 60000));
    ...
    public void saveFarmersBatch(Collection<Farmer> farmers) throws SQLException {
        ...
        String farmerSql = "INSERT INTO hukum_farmers " +
                ...
                "auto_harvest = VALUES(auto_harvest), " +
        ...
        String insertStorageSql = "INSERT INTO hukum_farmer_storage (farmer_id, crop_id, amount) VALUES (?, ?, ?) " +
    ...
    public void savePlayerLanguage(UUID playerUUID, String langCode) throws SQLException {
        ...
        String sql = "INSERT INTO hukum_player_preferences (player_uuid, language) VALUES (?, ?) " +
